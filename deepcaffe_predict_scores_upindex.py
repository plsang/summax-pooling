...
if __name__=="__main__":
	...
	for ii in range(start_video, end_video):
		...
		output_scores_update_file = '{}/{}.{}deepcaffe.scores.update.npy'.format(output_vdir, vid, nc)
		output_sim_file = '{}/{}.{}deepcaffe.sims.npy'.format(output_vdir, vid, nc)
		
		if not os.path.exists(output_scores_file):
			raise Exception( 'File [{}] not exists'.format(output_scores_file) );
				
		scores = np.load(output_scores_file)
		
		#sort keyframes
		sorted_idx = [i[0] for i in sorted(enumerate(kfs), key=lambda x:x[1])]
		...
		np.save(output_scores_update_file, scores)
		
		#for idx, val in enumerate(sims):
		#	print idx, val
		#subprocess.call(['chmod', '-R', '777', output_vdir])
		output_scores_txtfile = '{}/{}.{}deepcaffe.scores.txt'.format(output_vdir, vid, nc)
		#if os.path.exists(output_scores_txtfile):
		#	continue
		
		with open(output_scores_txtfile, 'w') as fh:
			for ii in range(0, scores.shape[0]):
				top_indices = (-scores)[ii,:].argsort()[:5]
				top_preds = labels[top_indices]
				top_scores = scores[ii,top_indices]
				#print ["%s: %.4f" % (u, v) for u, v in zip(top_preds, top_scores)]
				
				for u, v in zip(top_preds, top_scores):
					fh.write("[{}:{}] -- ".format(u, v))
				fh.write("\n");
