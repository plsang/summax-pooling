function video_pooling_simcat(proj_name, szPat)
    ...
    parfor ii = 1:length(segments),
		...
		code_sum_max_agg = zeros(codebook_size, length(thresholds));
		code_max_sum_agg = zeros(codebook_size, length(thresholds));
		...
		for jj=1:length(thresholds),
			...
			%l2 norm
			code_sum_max_jj = code_sum_max_jj / norm(code_sum_max_jj, 2);
			code_max_sum_jj = code_max_sum_jj / norm(code_max_sum_jj, 2);
			
			code_sum_max_agg(:, jj) = code_sum_max_jj;
			code_max_sum_agg(:, jj) = code_max_sum_jj;
		end
		
		code_sum_max_agg = reshape(code_sum_max_agg, codebook_size*length(thresholds), 1) ;
		code_max_sum_agg = reshape(code_max_sum_agg, codebook_size*length(thresholds), 1) ;
		
		output_sum_max_agg_dir = sprintf('%s/%s/%s.agg.summax.cat.top4.l2pn/%s', fea_dir, segment_ann, feature_ext, szPat) ;
		output_max_sum_agg_dir = sprintf('%s/%s/%s.agg.maxsum.cat.top4.l2pn/%s', fea_dir, segment_ann, feature_ext, szPat) ;
	
		if segnorm == 1,
			output_sum_max_agg_dir = sprintf('%s/%s/%s.agg.segnorm.summax.cat.top4.l2pn/%s', fea_dir, segment_ann, feature_ext, szPat) ;
			output_max_sum_agg_dir = sprintf('%s/%s/%s.agg.segnorm.maxsum.cat.top4.l2pn/%s', fea_dir, segment_ann, feature_ext, szPat) ;
		end
		
		%power norm
		code_sum_max_agg = sign(code_sum_max_agg) .* sqrt(abs(code_sum_max_agg)); 
		code_max_sum_agg = sign(code_max_sum_agg) .* sqrt(abs(code_max_sum_agg)); 
		
		par_save(output_sum_max_agg_dir, info.video, code_sum_max_agg);
		par_save(output_max_sum_agg_dir, info.video, code_max_sum_agg);		
